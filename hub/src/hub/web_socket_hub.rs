...
type Client = Arc<Mutex<WebSocketStream<TcpStream>>>;
type ClientsById = Arc<Mutex<HashMap<u32, Client>>>;
...
impl WebSocketHub {
    ...
    pub async fn start(&self) -> Result<(), Error> {
        ...
        while let Ok((stream, _)) = listener.accept().await {
            ...
            tokio::spawn(async move {
                let client_id =
                    Self::handle_new_client(&clients_by_id_clone, &client, &next_client_id_clone)
                        .await;
                Self::listen_to_client(&client, &client_id).await;
    ...
    async fn listen_to_client(client: &Client, client_id: &u32) {
